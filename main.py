...
class frame:
    ...
    def textbox(self):
        ...
        self.list=[] # * Tasked packed on display frame ~ display_frame(task displaying frame)
    ...
    def buttons(self):
        ...
        self.show_db=tk.Button(self.entry_frame,text='Done Task',padx=20,pady=10,command=self.show_task)
        self.show_db.place(relx=0,rely=0.15)
        # *delete button (deletes all the data from database)
        self.delete_db_data=tk.Button(self.entry_frame,text='Delete Data',padx=20,pady=10,command=self.deletedata)
    ...
    def todo_list(self):
        ...
        self.database(self.task) # ? call database functions and insert value(data,start_time,todoname)
    ...
    def show_task(self):
        ...
        # * Lables for database
        self.date_lb=tk.Label(self.show_window,text='Date',padx=30,pady=10)
        self.startt_lb=tk.Label(self.show_window,text='Start',padx=30,pady=10)
        self.task_lb=tk.Label(self.show_window,text='Task',padx=30,pady=10)
        ...
        for i in range(4):
            for j in range(1,len(self.mydb_items)):
                self.label=tk.Label(self.show_window,text=self.mydb_items[j][i],padx=30,pady=10)
